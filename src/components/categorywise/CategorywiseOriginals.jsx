...
function CategorywiseOriginals() {
  ...
  useEffect(() => {
    ...
    if (query === '') {
      ...
      axios.get(Originals + '&page=' + page).then((response) => {
      ...
      axios.get(`/search/tv?api_key=${API_KEY}&query=${query}&page=${page}`).then((response) => {
...
export default CategorywiseOriginals
